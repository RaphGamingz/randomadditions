...
public class Bounctium extends BreakableBlock {
	public Bounctium() {
		super(Block.Properties
				.create(Material.CLAY, MaterialColor.LIGHT_BLUE)
				.slipperiness(0.8F)
        		.harvestLevel(3)
				.sound(SoundType.SLIME)
        );
        setRegistryName(ModRegistries.location("bounctium"));
	}
	...
	public void onLanded(IBlockReader worldIn, Entity entityIn) {
	   if (entityIn.isSneaking()) {
         ...
         if (vec3d.y < 0.0D) {
        	double d0 = entityIn instanceof LivingEntity ? 5.0D : 4.0D;
